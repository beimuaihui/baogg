...
class Db
{
    ...
    public static function getDb($key, $debug = false): ExtendedPdo
    {
        ...
        if (!array_key_exists($key, self::$arr_db)) {
            ...
            self::$arr_db[$key] =  new ExtendedPdo(
                ...
                [
                    ...
                    \PDO::ATTR_STRINGIFY_FETCHES => (isset($db['force_string_column']) && $db['force_string_column']) ? true : false,
                ], // driver attributes/options as key-value pairs
                ["SET NAMES '{$db['charset']}'"]  // queries to execute after connection
            ); //
    ...
    public static function getSlaveDb($key): ExtendedPdo
    {
        $key = isset(self::$key_map[$key]) ? self::$key_map[$key] : strtolower($key);
        ...
        if (!array_key_exists($key, self::$arr_slave_db)) {
            ...
            try {
                ...
                $row_slave =  $arr_slaves[$index_slave_db];

                self::$arr_slave_db[$key] =  new ExtendedPdo(
                    $db['driver'] . ':host=' . $row_slave . ';dbname=' . $db['database'],
                    $db['username'],
                    $db['password'],
                    [\PDO::ATTR_ERRMODE => \PDO::ERRMODE_EXCEPTION, \PDO::ATTR_DEFAULT_FETCH_MODE, \PDO::FETCH_ASSOC], // driver attributes/options as key-value pairs
                    ["SET NAMES '{$db['charset']}'"]  // queries to execute after connection
                );

                $row_status = self::$arr_slave_db[$key]->fetchOne("SHOW SLAVE STATUS");
     ...
     * @return string
     */
    public static function isDefaultDbKey($key = '')
    {
        $key = isset(self::$key_map[$key]) ? self::$key_map[$key] : strtolower($key);
        ...
        return $c['settings']['db'][$key]['is_default'];
    }
     ...
     * @return string
     */
    public static function getDefaultDbKey()
    {
        // $key=isset(self::$key_map[$key])?self::$key_map[$key]:strtolower($key);
