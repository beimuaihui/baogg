...
class Expression
{
    public function testing()
    {
        ...
        return $this->parse($expr); // (1 and 2)  or (3 and 4) or (5 and 6)
    }

    /**
     * 根据字符串表达转成数组
     *
     * @param string  $expr  表达式，如"1 and 6 or 2 and 3 or 4 and 5" => array(or => ([and => (1,6)],[ and => (2,3)], ...)
        ...
        if (strpos($expr, 'or') !== false) {
            ...
            foreach ($arr_expr as $k_sub_expr => $v_sub_expr) {
                ...
                if ($v_sub_expr < 0) {
                    ...
                    $arr_expr[$k_sub_expr] = $this->parse(array_search(0 + $v_sub_expr, $arr_sub_group_replace), $level++);
                }
            }
...
$arr = $Expression->testing();
echo __FILE__.__LINE__." \n arr = ".var_export($arr, true);

$str = $Expression->stringify($arr);
echo __FILE__.__LINE__." \n res = ".$str;
